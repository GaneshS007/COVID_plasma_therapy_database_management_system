...
<head>
    ...
    <title>Website for Blood Bank to deliver the plasma to the Hospital</title>
...
<body>
	...
	<%
		...
		{
			loggedInBbankId = (int) session.getAttribute("BbankId");
		}
	%>
	Welcome ${adminBB}	
	...
	if(request.getParameter("request") != null && request.getParameter("donation") != null)
	{
		...
		String updateDeliver="update request set request_status='delivered' where request_id="+request_id+" and  donation_id ="+donation_id+" and request_status='requested'";
		if (dao.putData(updateDeliver) > 0)
		{
			String updateDeliver1 = "update donation set Hospital_request_status ='delivered' where donation_id ="+donation_id+" and Hospital_request_status='requested'";
                    ...
                    <div class="col-md-9 nav-col">
                        <nav class="navbar navbar-expand-lg navbar-light">
                            ...
                            <div class="collapse navbar-collapse" id="navbarNav">
                                <ul class="navbar-nav">
                                           <li class="nav-item">
                                        <a class="nav-link" href="../../BloodBank/Home1/Home1.jsp">Back to Blood Bank</a>
                                    </li>

                                    <li class="nav-item">
                                        <a class="nav-link" href="#mytable">Plasma Requested</a>
                                    </li>
                                </ul>
                            </div>
                        </nav>
...
<br>
    	  ...
    	  <table id="mytable" class="table">
    	        <thead class="thead-dark">
    	          <tr>
				      ...
				      <th scope="col">Qty of plasma in Units 1u=200ml</th>
    	...
    	<%
		String plasmaRequested = "SELECT request.request_id, request.donation_id, hospital.Name, donation.Blood_Group, donation.Quantity_of_plasma, donation.IgM, donation.IgG, donation.Hospital_request_status FROM request, donation, hospital, blood_bank WHERE donation.Bbank_id = "+loggedInBbankId+" AND blood_bank.Bbank_id = "+loggedInBbankId+" AND request.donation_id = donation.donation_id AND request.Hospital_id = hospital.Hospital_id AND donation.Hospital_request_status <> 'Not Requested'";
        ResultSet rs = dao.getData(plasmaRequested);
        %><tbody>
        <%
        while(rs.next())
        {
        %>
            <tr>
              ...
              <td><%=rs.getFloat("IgM") %></td>
      		  <td><%=rs.getFloat("IgG") %></td>
              <% 
